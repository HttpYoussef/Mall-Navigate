# Phase 3 Execution Plan — Sensor Fusion Boundary
...
- Production of the **Corroboration Signal** (PDR displacement vector) for future consumption by Module 8.
- Production of the **Fallback Displacement** estimate for future consumption by Module 6.
...
- Modification of `NavigationSessionManager` or `NavSessionState` (unless strictly required for data availability).
...
| `NavConfig` | `com.example.mallar.navigation.NavConfig.kt` | **Reuse** | Physical constants (Default Stride Length, Stale Window). |
...
- **Interface**:
    - `getCorroborationSignal()`: Returns the current accumulated PDR vector and resets the accumulator (or provides a delta since last call).
    - `getFallbackDisplacement()`: Returns the displacement since the last confirmed tracking state.

### 4. Technical Validation (Unit Tests)
- **Listener-Free Check**: Verify no `SensorEventListener` is implemented or registered.
...
3. **Drift Verification**: Assert that PDR displacement remains within reasonable bounds for a simulated path.
